...
public class SnakeView extends TileView {
     ...
     * Draws some walls.
     */
    private void updateWallsAnr(int whichTiles) {

        int useTile;
        if (whichTiles == 0) {
            useTile = RED_STAR;
        } else if (whichTiles == 1) {
            useTile = YELLOW_STAR;
        } else {
            useTile = GREEN_STAR;
        }
        ...
        for (int x = 0; x < mXTileCount; x++) {
            setTile(useTile, x, 0);
            setTile(useTile, x, mYTileCount - 1);
        }
        for (int y = 1; y < mYTileCount - 1; y++) {
            setTile(useTile, 0, y);
            setTile(useTile, mXTileCount - 1, y);
        }
    }
    /* @@ ORIG
    ...
    /** Simple class having x,y. (Use some builtin class?)
     */
